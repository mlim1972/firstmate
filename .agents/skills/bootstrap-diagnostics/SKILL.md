...
- `MISSING: contribution observation could not be armed; coverage is unconfirmed` - distinct from the tool-install `MISSING:` line above; there is no tool to install here.
  `fm-contributions.sh arm --if-owned` (called from `bin/fm-bootstrap.sh`) failed to register the authenticated custom check that polls this home's owned contributions; `bin/fm-check-register.sh` owns the permission and device checks that can reject the registration.
  Inspect `state/contributions.check.sh` and its trust file for a symlink, wrong permissions, or wrong-device placement, fix it, then rerun session start; do not treat this as a missing dependency to install.
